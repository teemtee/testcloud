...
class Workarounds():
    ...
    def _populate_defaults(self) -> None:
        self.add("sed -i -e '/^.*PermitRootLogin/s/^.*$/PermitRootLogin yes/'"\
        ...
        self.add_condition('[ ! -f /etc/systemd/network/20-tc-usernet.network ] && systemctl status systemd-networkd | grep -q "enabled;\\svendor\\spreset:\\senabled"',
                           ...
                           key="user_net_config")
        self.add_condition('systemctl status systemd-networkd | grep -q "enabled;\\svendor\\spreset:\\senabled"',
    ...
    def _parse_cmd(self, cmd:str|list[str]) -> str:
        if isinstance(cmd, str):
            return cmd
        else:
            return " && ".join(cmd)

    def add(self, cmd:str|list[str], key:Optional[str]=None) -> None:
        cmd = self._parse_cmd(cmd)
        key = self._generate_key() if not key else key
        self._workarounds[key] = self._create_workaround_cmd(cmd)

    def add_condition(self, condition:str, cmd:str|list[str], key:Optional[str]=None):
        sh_condition_string = "if %s ; then %s ; fi"
        cmd = self._parse_cmd(cmd)
        self.add(sh_condition_string % (condition, cmd), key=key)

    def add_os(self, os:str, cmd:str|list[str], key:Optional[str]=None) -> None:
